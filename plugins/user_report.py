...
async def Report_Function(No):
    ...
    if return_code == 0:
        ...
        print(stderr)
        return [f"<b>𝐒‌σϻєᴛʜɪηɢ 𝐖‌єηᴛ 𝐖‌ꝛσηɢ 𝐊‌ɪηᴅʟʏ 𝐂‌ʜєᴄᴋ ʏσυꝛ 𝐈‌ηᴘυᴛs 𝐖‌ʜєᴛʜєꝛ 𝐘‌συ 𝐇‌ᴧᴠє 𝐅‌ɪʟʟєᴅ 𝐂‌σꝛꝛєᴄᴛʟʏ σꝛ 𝐍‌σᴛ !</b>\n\n<code>{stderr.decode('utf-8', 'ignore')}</code>\n\n𝐄‌ꝛꝛσꝛ", False]

async def CHOICE_OPTION(bot, msg, number):

    if not config_path.exists():
        return await msg.reply_text(text="**𝐘‌συ ᴅση'ᴛ ʜᴧᴠє ᴧηʏ ᴄσηғɪɢ ғɪꝛsᴛ ϻᴧᴋє ᴛʜє ᴄσηғɪɢ ᴛʜєη ʏσυ'ʟʟ ᴧʙʟє ᴛσ ꝛєᴘσꝛᴛ**\n\n➜ 𝐔‌sє /make_config", reply_to_message_id=msg.id, reply_markup=ReplyKeyboardRemove())
    ...
    try:
        if Path('report.txt').exists():
            return await msg.reply_text(text="**𝐀‌ʟꝛєᴧᴅʏ 𝐎‌ηє 𝐏‌ꝛσᴄєss ɪs 𝐎‌ηɢσɪηɢ 𝐏‌ʟєᴧsє 𝐖‌ᴧɪᴛ 𝐔‌ηᴛɪʟ ɪᴛ's 𝐅‌ɪηɪsʜєᴅ ⏳**", reply_to_message_id=msg.id)
        ...
        await bot.send_message(msg.from_user.id, "𝐄‌ꝛꝛσꝛ!!\n\n𝐑‌єǫυєsᴛ ᴛɪϻєᴅ συᴛ.\n𝐑‌єsᴛᴧꝛᴛ ʙʏ υsɪηɢ /report")
        return

    ms = await bot.send_message(chat_id=msg.chat.id, text=f"**𝐏‌ʟєᴧsє 𝐖‌ᴧɪᴛ**\n\n➜ 𝐇‌ᴧᴠє 𝐏‌ᴧᴛɪєηᴄє ⏳", reply_to_message_id=msg.id, reply_markup=ReplyKeyboardRemove())
    if str(no_of_reports.text).isnumeric():
        ...
        while i < int(no_of_reports.text):
            try:
                ...
                await msg.reply_text(text=f"**𝐀‌η υηєxᴘєᴄᴛєᴅ єꝛꝛσꝛ σᴄᴄυꝛꝛєᴅ ɪη 𝐑‌єᴘσꝛᴛ_𝐅‌υηᴄᴛɪση:\n\n`{e}`**\n\n𝐄‌ꝛꝛσꝛ !")
        ...
        await msg.reply_text(text='**𝐏‌ʟєᴧsє 𝐄‌ηᴛєꝛ 𝐕‌ᴧʟɪᴅ 𝐈‌ηᴛєɢєꝛ 𝐍‌υϻʙєꝛ !**\n\n➜ 𝐓‌ꝛʏ 𝐀‌ɢᴧɪη :- /report')
        return
